...
TasQueueDisc::DoEnqueue (Ptr<QueueDiscItem> item)
{
  ...
  if(item->GetPacket ()->PeekPacketTag (m_priotag))
    {
      childclass = m_priotag.GetPriority () & 0x7;
    }
  else
    {
      ...
      if (childclass == PacketFilter::PF_NO_MATCH)
          {
            NS_LOG_LOGIC ("tas-queue-disc: No filter was able to classify this packet.");
            childclass = 0;
          }
        else
          {
            childclass = childclass & 0x7;
            NS_LOG_LOGIC ("Childclass for sorting " << childclass);
          }
    }
 
  m_priotag.SetPriority(childclass);
  item->GetPacket()->RemovePacketTag(m_priotag);
  item->GetPacket()->AddPacketTag(m_priotag);
  ...
  NS_LOG_LOGIC ("Paket enqueued in " << (int)m_prioMap[childclass] << ". Pakets in Queue: " << GetQueueDiscClass(m_prioMap[childclass])->GetQueueDisc ()->GetNPackets () << ". Time: " << now << '\n');
          ...
          if(returnVal != 0)
            {
              ...
              NS_LOG_LOGIC("Paket Dequeued at time: " << now );
              return returnVal;
            }
        }
    }
     ...
     if(m_gateListConfig.at(index).GetGateState() != gateState || m_gateListConfig.at(index).GetNextEvent() != nextEvent)
       {
         ...
         NS_LOG_LOGIC(this << " Queue: " << index <<" NewState: " << newState.state);
         //NS_LOG_LOGIC(this << " Queue: " << index <<" NewState: " << newState.state << " Interval: " << newState.interval.GetMilliSeconds() << "ms");
...
TasQueueDisc::SetTransmitting(Ptr<const QueueDiscItem> item)
{
  ...
  NS_LOG_LOGIC("Transmitting queued packet at " << Simulator::Now() + sleepTime);
...
TasQueueDisc::InitializeParams (void)
{
  ...
  switch (GetNQueueDiscClasses()){
    ...
    case (8):m_prioMap={0,1,2,3,4,5,6,7}; break;
  }
  m_StoppAllQueues = false;
}
